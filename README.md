...
Because of the android support library fragment lifecycle issues we have to still inflate to cover the
use case of defining fragments by xml. So we pass context which is actually activity and use the
onCreate method ot than call our CreateByInflator method. We use a shortcut set of logic
in our search path forboth defined widgets and our own custom widgets.

This should not present a problem unless you are unconfortable using logging and debuging and small concise iterations.
